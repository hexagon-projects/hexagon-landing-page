  ...
  <div class="relative grid items-center gap-8 px-6 sm:grid-cols-1 md:grid-cols-2 md:px-32" id="our">
    ...
    <div v-if="Array.isArray(items[selected].content)" class="z-10 p-6 space-y-4">
      ...
      <div v-for="(content, index) in items[selected].content" :key="index"
        class="flex items-center gap-4 p-4 rounded-lg backdrop-blur-sm bg-white/50 dark:bg-gray-800/50">
        <!-- Tampilkan Title -->
    ...
    <!-- Right Section -->
    <div v-else
      class="z-10 p-6 space-y-4 rounded-lg bg-white/50 backdrop-blur-md dark:bg-gray-800/50 min-h-[300px] flex flex-col justify-between">
      <!-- Judul -->
